...
export default function StudentProfilePage({
    ...
    return (
      <div className="min-h-screen bg-[#f0f4f3] flex items-start justify-center pt-20">
        <div className="max-w-[480px] w-full mx-5 bg-white rounded-[14px] border border-[#ffd0d0] p-9 text-center">
          ...
          <h2 className="text-lg font-bold text-[#0d1f1a] mb-2">Student record not found</h2>
  ...
  // Card is expired if the student is inactive, OR if the academic year
  // has ended and is no longer marked active.
  const expired =
    !student.is_active ||
    (student.academic_years?.end_date != null &&
      !student.academic_years.is_active &&
      new Date(student.academic_years.end_date) < new Date());
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f3] text-[#0d1f1a] font-sans selection:bg-[#1aaa85] selection:text-white pb-12">
      ...
      {/* TOP NAV */}
      <nav className=" px-4 md:px-8 h-[58px] flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center gap-2.5">
          <div className="w-30 h-30  flex items-center justify-center overflow-hidden p-1">
          ...
          {/* <div>
            ...
            <div className="text-[11px] text-white/70 font-normal hidden md:block">Student Verification Portal</div>
          </div> */}
        </div>
          ...
          <aside className="flex flex-col gap-3.5">
            <div className="bg-white rounded-[14px] border border-[#e0ebe8] overflow-hidden fade-in delay-1">
              ...
              <div className="px-5 pb-5 -mt-9 relative z-20">
                <div className="text-[20px] font-extrabold text-center mb-1 mt-2.5 text-[#0d1f1a]">{student.full_name}</div>
                <div className="text-[12px] text-[#6b8480] text-center mb-3.5">
                  Class {cls} · {academicYear}
                </div>
                ...
                <div className="flex items-center justify-center gap-1.5 bg-[#e1f5ee] rounded-full py-1.5 px-3.5 mb-3.5">
                  ...
                  <span className="text-[13px] font-bold text-[#0f7a60] font-mono tracking-wide">
                    {student.studentid || student.admission_no}
                  </span>
                </div>

                <div className="flex flex-wrap gap-1.5 justify-center mb-4.5 mb-4">
                  <span className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-[#e1f5ee] text-[#0f7a60]">
                    <div className="w-2.5 h-2.5 fill-[#0f7a60]"><Icon.Check /></div> {student.is_active ? 'Active' : 'Inactive'}
                  </span>
                  {student.blood_group && (
                    <span className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-semibold bg-[#fff0f0] text-[#c0392b]">
                      <div className="w-2.5 h-2.5 fill-[#c0392b]"><Icon.Heart /></div> {student.blood_group}
                    </span>
                  )}
                  {section !== "—" && (
                ...
                <div className="flex flex-col">
                  <div className="flex items-center gap-2.5 py-2 border-b border-[#e0ebe8]">
                    ...
                    <div>
                      ...
                      <div className="text-[13px] text-[#0d1f1a] font-medium mt-px">{student.full_name}</div>
                    </div>
                  </div>
                  {student.date_of_birth && (
                    <div className="flex items-center gap-2.5 py-2 border-b border-[#e0ebe8]">
                      ...
                      <div>
                        ...
                        <div className="text-[13px] text-[#0d1f1a] font-medium mt-px">{formatDate(student.date_of_birth)}</div>
                      </div>
                    </div>
                  )}
                  {student.phone && (
                    <div className="flex items-center gap-2.5 py-2 border-b border-[#e0ebe8] last:border-b-0">
                      ...
                      <div>
                        <div className="text-[10px] text-[#a8bdb9] font-semibold uppercase tracking-wider">Parent contact</div>
                        <div className="text-[13px] text-[#0d1f1a] font-medium mt-px">{student.phone}</div>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="bg-[#f7faf9] border border-[#e0ebe8] rounded-lg px-4 py-3.5">
                    <div className="text-[10px] text-[#a8bdb9] font-semibold uppercase tracking-wider flex items-center gap-1.5 mb-1">
                      <div className="w-3 h-3 fill-[#1aaa85]"><Icon.Star /></div> Class
                    </div>
                    <div className="text-[15px] font-bold text-[#0d1f1a]">{cls}</div>
                  </div>

                  {student.blood_group && (
                    <div className="bg-[#f7faf9] border border-[#e0ebe8] rounded-lg px-4 py-3.5">
                      <div className="text-[10px] text-[#a8bdb9] font-semibold uppercase tracking-wider flex items-center gap-1.5 mb-1">
                        <div className="w-3 h-3 fill-[#1aaa85]"><Icon.Heart /></div> Blood group
                      </div>
                      <div className="text-[15px] font-bold text-[#0d1f1a]">{student.blood_group}</div>
                    </div>
                  )}
                  <div className="bg-[#f7faf9] border border-[#e0ebe8] rounded-lg px-4 py-3.5">
                    <div className="text-[10px] text-[#a8bdb9] font-semibold uppercase tracking-wider flex items-center gap-1.5 mb-1">
                      <div className="w-3 h-3 fill-[#1aaa85]"><Icon.Calendar /></div> Academic year
                    </div>
                    <div className="text-[15px] font-bold text-[#0d1f1a]">{academicYear}</div>
                  </div>
                </div>
              </div>
            </div>
